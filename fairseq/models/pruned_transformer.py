...
@register_model("pruned_transformer")
class PrunedTransformerModel(FairseqEncoderDecoderModel):
        """
        ...
        def apply_masks_grad_hooks(self):
                """
                Applies masks by setting masked weights to zero,
                and also creating backwards hooks that masks weights during the backward pass.
                """
                for name, param in self.encoder.named_parameters():
                        mask_name = PrunedTransformerModel.to_mask_name(name, is_encoder=True)
                        if mask_name in self.masks:
                                param.data *= self.masks[mask_name]
                                param.register_hook(lambda x, name=mask_name: x*self.masks[name])
                for name, param in self.decoder.named_parameters():
                        mask_name = PrunedTransformerModel.to_mask_name(name, is_encoder=False)
                        if mask_name in self.masks:
                                param.data *= self.masks[mask_name]
        ...
        def random_prune(self, prune_frac):
                """
                Randomly prune a fraction of remaining weights.
                prune_frac specifies the percent of the *current* number of weights to prune.
                e.g. prune_frac = 0.2 when we have 40% sparsity means that an additional 
                0.2*60% = 12% of the total number of initial weights will be pruned.
                """
        ...
        def get_sparsity(self):
                ...
                for name, param in self.decoder.named_parameters():
                        ...
                        if mask_name in self.masks:
                                total_masked += (self.masks[mask_name] == 0).sum()
                return [int(total_params), int(total_masked), float(1.0 * total_masked / total_params)]
...
class TransformerDecoder(FairseqIncrementalDecoder):
        ...
        def extract_features(
                ...
                for idx, layer in enumerate(self.layers):
                        ...
                        x, layer_attn, _, decdec_attn = layer(
                                x,
                        ...
                        encdec_attns.append(layer_attn)
                        decdec_attns.append(decdec_attn)
                ...
                return x, {"attn": [attn], "inner_states": inner_states, "encdec_attns": encdec_attns, "decdec_attns": decdec_attns}
...
@register_model_architecture("pruned_transformer", "pruned_transformer_vaswani_wmt_en_de_big")
